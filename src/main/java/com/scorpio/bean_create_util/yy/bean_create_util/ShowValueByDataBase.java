...
//			String className = initNameByTable(tableName, true);
			...
			if (!isChangeVoPackageName) {
				voPackageName = voPackageName.substring(0, voPackageName.length() - 3) + "po;";
				isChangeVoPackageName = true;
			}
			childClazzBuf.append("import java.io.Serializable;\r");
			childClazzBuf.append("public class " + baseName + "Vo extends " + voPackageName.substring(0, voPackageName.length() - 1) + "." + baseName + "Po implements \r\t\tSerializable {\r\r");
...
//		String className = initNameByTable(tableName, true);
		...
		clazzBuf.append("public class " + baseName + "Po {\r\r");
...
//		clazzBuf.append(methodBuf.toString());
		...
		writeFile(baseName, voFolder, packageBuf, "Po.java");
...
//		System.out.println("delete from " + tableName + ";");
		...
		System.out.println("\t<mapper resource=\"cn/com/enorth/exam/xml/" + baseName + "Mapper.xml\"/>");
